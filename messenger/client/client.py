...
class MessageHandler(BaseHTTPRequestHandler):
    ...
    def _send_message(self, content: str) -> dict:
        ...
        message_request = google.protobuf.json_format.ParseDict(
            json_request,
            messenger_pb2.MessageRequest()
        )
        response = self._stub.SendMessage(message_request)
...
def consume_messages(stub, postbox):
    ...
    for message in stub.ReadMessages(empty, wait_for_ready=True):
        message_dict = google.protobuf.json_format.MessageToDict(
            message, 
        )
        postbox.put_message(message_dict)
...
def main():
    ...
    # Implement and run a messages stream consumer in a background thread here.
